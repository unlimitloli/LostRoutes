...
string MyUtility::getUTF8Char(const string key)
{
	...
	string sharePath = shareFileUtils->fullPathForFilename("utf8_char.plist");

	bool isExist = false;
	isExist = shareFileUtils->isFileExist(sharePath);
	if (!isExist)
	{
	...
	ValueMap dict = shareFileUtils->getValueMapFromFile(sharePath);
